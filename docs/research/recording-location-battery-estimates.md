...
## Field observation: pre-Activity Recognition transition baseline

On 2026-07-17, a roughly 30-minute recorded run consumed about 5 percentage points of displayed phone battery: approximately 10 percentage points per hour if that short observation were extrapolated. This is a useful real-device baseline, not a battery benchmark: device model, display state, signal conditions, other apps, temperature, and battery-health context were not captured.

The run occurred **before** the Activity Recognition transition-intent implementation. It must not be used to judge the battery behavior of the current activity-adaptive recording policy. Repeat comparable screen-off and map-on runs after that implementation is exercised on-device.

On 2026-07-18, **after the Activity Recognition transition-intent implementation was deployed**, a Recording Session was accidentally left active for an estimated 3–4 hours and consumed about 5 percentage points of displayed battery. The device was stationary for most of that period. This is encouraging real-device evidence that the activity-adaptive build is much cheaper while stationary than the pre-transition running baseline, but it does not establish the exact polling state or attribute the saving solely to Activity Recognition; the session’s actual activity transitions, screen state, signal conditions, and other battery consumers were not recorded.
